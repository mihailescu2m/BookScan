...
def build(src, dst, W, H, crop_h, headline, subhead, head_px, sub_px):
    img = Image.open(src).convert("RGB")
    cap_h = H - crop_h
    shot = img.crop((0, 0, W, crop_h))
...
heads = [
    "Scan it to add it in your library",
    "See where each is placed",
    "Scan it to find its place",
    "Lend it to a friend",
    "Scan it to put it back",
]
subs = [
...
for i, name in enumerate(files):
    build(f"{base}/iphone-6.9/{name}.png", f"{base}/appstore/iphone-6.9/{name}.png",
          1320, 2868, 2286, heads[i], None, 100, 0)
    build(f"{base}/ipad-13/{name}.png", f"{base}/appstore/ipad-13/{name}.png",
          2064, 2752, 2175, heads[i], subs[i], 108, 58)
